...
class spatial_ODE_SIR(ODE):
    """
    SIR model with an age and spatial stratification
    """
...
class spatial_ODE_SIR_tf(ODE):
    ...
    def integrate(t, S, I, R, beta, gamma, f_v, N, M):
        ...
        N = tf.convert_to_tensor(N, dytpe=float)
        M = tf.convert_to_tensor(M, dytpe=float)
        I_rel = tf.convert_to_tensor(I/(S+I+R), dytpe=float)
...
class spatial_TL_SIR(JumpProcess):
    """
    Stochastic SIR model with an age and spatial stratification
    """
    states = ['S', 'I','R']
    parameters = ['beta','gamma', 'f_v', 'N', 'M']
    dimensions = ['age', 'location']
